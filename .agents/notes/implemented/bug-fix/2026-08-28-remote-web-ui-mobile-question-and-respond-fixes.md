...
1. **Question options missing under weak-network polling**:
   ...
   - The 1.5s fallback polling endpoint mobile.pending returns PendingQuestionGroup[] nested under each \rpcId ({ rpcId, questions: [...] }).
...
2. **Protocol mismatch in mobile respond submission (agent hangs indefinitely)**:
   ...
   - The host handler MOBILE_RESPOND_METHOD directly read payload.rpcId and payload.response (undefined), causing \apiProxy.respond to reject with
...
- **Host protocol translation**:
  - In src/mobile-api.ts, look up the matching \rpcId from pendingTracker for the given \approvalId or question ids.
...
- **Flattening questions directly inside PendingTracker**: Rejected — \rpcId is the single source of truth for host-side RPC lifecycles; stripping it at the tracker level would hinder RPC-level lookups.
