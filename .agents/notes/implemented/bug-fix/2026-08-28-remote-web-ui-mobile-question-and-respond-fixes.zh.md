...
1. **弱网/兜底轮询下提问选项不渲染**：
   ...
   - 在弱网或 SSE 断线触发的 1.5s 轮询兜底中，\fetchPending 返回的是带有外层 \rpcId 包装的 PendingQuestionGroup[]（即 { rpcId, questions: [...] }）。
...
2. **移动端审批与提问提交协议不匹配（Agent 挂起不解冻）**：
   ...
   - Host 端 MOBILE_RESPOND_METHOD 原实现直接读取 payload.rpcId 和 payload.response（均为 undefined），导致 \apiProxy.respond 因未找到匹配项返回
...
- **Host 侧适配与协议翻译**：
  - 在 src/mobile-api.ts 的 MOBILE_RESPOND_METHOD 分支中，根据 	ype（\approval / question）从 pendingTracker 反查该会话下对应 \approvalId 或包含对应问题 ID 的挂起帧 \rpcId。
...
- **修改 PendingTracker 直接返回扁平结构**：否决——\rpcId 是 Host 侧多路复用与请求生命周期的事实源，若在 tracker 层面抹除 \rpcId 会破坏其他需要精确定位具体 RPC 请求的场景。
