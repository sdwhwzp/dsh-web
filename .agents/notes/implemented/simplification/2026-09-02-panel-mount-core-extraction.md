...
The takeover lifecycle now lives exactly once in `shared/client/panel-mount-core.ts`: `mountCenterPanel(options)` owns the column selector, the MutationObserver remount, the eviction-and-activate sequence, the sidebar click-out listener, and the disposer order; the `CenterPanelMountOptions` contract carries the per-plugin parameters (panel tree, view dataset key, semantic plugin name, CSS class, the plugin's own html active attribute, the panel name) plus the controller subscribe and the optional locale source. The file joins the sync-shared manifest with two generated copies at extraction time (`packages/dsh-ssh/src/client/panel-mount-core.ts`, `packages/dsh-task-board/src/client/panel-mount-core.ts`); the sync-shared test's copy-count buckets move 112→114 total and 41→43 client. Both wrappers shrink to the parameter wiring (~45 lines each) and keep their public exports (`mountPanel` + `PANEL_VIEW_SELECTOR`, `mountBoard` + `BOARD_VIEW_SELECTOR`), so consumer code and tests are untouched. The rebuilt aggregate client bundle (`packages/dsh-web-all/lib/client.js`) ships the same behavior inlined from source.

Occupancy across panels moved from per-panel sibling parameters to the shared `PANEL_FAMILY` table when the skill center became a third center-column panel: see [One center-column panel family](../../architecture/2026-09-23-center-column-panel-family.md). That shared file now has three generated consumers (`dsh-skill-explorer/src/client/panel-mount-core.ts` is the third), and the pairwise sibling options this note describes no longer exist.

Container attribute names stay wrapper-supplied parameters: they are pinned by each package's CSS (`panel.module.css` / `board.module.css` / the skill center's `panel.module.css` cross-reference the family's html attributes), the wallpaper-exclusive skin patches, and the semantic-attributes contract, so the extraction deliberately changes none of them.
