...
本 fork 使用上游 0.3.24 家族版本及 Harness 0.1.7-alpha.2 SDK。本地部署候选统一使用 `0.3.24-dsh.20260923.2`，避免重新构建家族时覆盖已有的不可变包版本。Pet 使用上游助手流投影，同时保留经过验证的身份存储、新账号默认关闭以及与宿主全局活动的隔离。已有账号、活动、销毁和奖励去重测试覆盖这些保留行为。
