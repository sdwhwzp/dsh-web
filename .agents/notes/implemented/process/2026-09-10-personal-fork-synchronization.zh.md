...
本 fork 使用上游 0.3.24 家族版本及 Harness 0.1.7-alpha.1 SDK。Pet 使用上游助手流投影，同时保留经过验证的身份存储、新账号默认关闭以及与宿主全局活动的隔离。已有账号、活动、销毁和奖励去重测试覆盖这些保留行为。

0.1.7 配置表单使用上游的 volatile Config 字段与按条目寻址的设置 API。Pet 将已认证的账号接口作为 `ConfigForm`，保留 revision 校验并返回写入接受／拒绝结果。本地宠物关闭时路由仍保持注册，其他账号可以开启自己的宠物。SSH 账号隔离仍是需要重新挂载的 profile 配置；显示设置使用上游实时引用。
...
市场自动部署仅在原作者仓库运行。本 fork 需要明确请求的 `workflow_dispatch`；同步推送只发布源码。工作流条件独立于部署凭据是否存在来执行此限制。
