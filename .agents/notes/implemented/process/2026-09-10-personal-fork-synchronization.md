...
The fork uses the upstream 0.3.24 family and Harness 0.1.7-rc.1 SDK. Local deployment candidates use the unified version `0.3.24-dsh.20260923.4` so rebuilding the family cannot replace an existing immutable package version. Pet consumes the upstream assistant stream projection while preserving verified-principal storage, disabled defaults for new accounts, and isolation from host-global activity. Existing account, activity, disposal, and reward-deduplication tests cover these retained behaviors.

The workspace SDK ranges, host peers, CI mount pins, and desktop host seed track 0.1.7-rc.1 together. The two existing SDK package extensions restore runtime dependencies omitted from the published client manifests. The aggregate declares the boot integration test’s Host peers as development dependencies, so these tests can load the cohort’s real boot implementation.
