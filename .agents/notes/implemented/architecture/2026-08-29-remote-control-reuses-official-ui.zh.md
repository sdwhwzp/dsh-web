...
- **通道门是无条件的。** `remote-api.ts` 的配对凭据校验不再参考 `requirePairingForLan`：通道会用进程自身的浏览器凭据重新发起每一次转发调用，放行未配对调用者等于把该权限交出去。局域网策略只决定桌面的客户端重写选择与普通 `/api` 面的姿态。见[通道门修复](../bug-fix/2026-09-21-issue-batch-1646-1665-channel-gate-sidebar-socket-and-label-race.zh.md)——它闭合了「配对设备经重新开放的设置桥写入该策略来解除门」这条链。
