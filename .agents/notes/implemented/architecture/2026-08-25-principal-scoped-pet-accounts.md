...
- The browser settings card reads `/api/pet/settings` and writes `/api/pet/settings/mutate`, so selection, display, visibility, account enablement, and decoration enablement use the same principal-scoped file as interactions, naming, and gameplay. Revision-fenced writes prevent concurrent tabs from silently replacing a newer value.
