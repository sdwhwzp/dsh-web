...
- Direct access on the Host port preserves the established pet and its full session-reactive behavior. Each authenticated gateway account starts disabled, can opt in independently, and persists its choice across browser reloads and service restarts; account renames retain the same pet because the immutable id remains stable.
...
- Coverage exercises persistence across reload, two-account independence including gameplay state, forged and tampered identity rejection, trusted-principal HTTP routing, revision-fenced settings writes, and the direct-account path.
