...
- The browser settings card reads `/api/pet/settings` and writes `/api/pet/settings/mutate`, so selection, display, visibility, account enablement, and decoration enablement use the same principal-scoped file as interactions, naming, skins, and gameplay. A principal with no pet file resolves to `enabled: false`, and the browser mounts neither the pet root nor its poll loop until the account settings response explicitly enables it. Enabling the pet writes only that principal's file. Revision-fenced writes prevent concurrent tabs from silently replacing a newer value.
...
- Principal-scoped companions do not consume host-global session activity or bubbles. The current session event stream has no verified principal attribution; returning an idle view prevents cross-account activity disclosure. Direct interactions, gameplay state, affinity, treats, names, selection, per-pet skins, layout, and switches remain account-specific.

The `set-skin` route passes the verified account scope to both the active-pet lookup and the persisted skin write. State reads resolve the stored skin against that account's selected pet; a skin removed from its manifest resolves to the default. Skin writes advance the account revision so an older settings mutation cannot overwrite newer state.
...
- Coverage exercises persistence across reload, two-account independence including gameplay state and persisted skins, forged and tampered identity rejection, trusted-principal HTTP routing, revision-fenced settings writes, and the direct-account path.
