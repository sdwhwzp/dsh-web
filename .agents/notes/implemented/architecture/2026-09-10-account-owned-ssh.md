...
Ordinary accounts supply passwords or inline private keys. The server's key files, SSH agents, local `ProxyCommand` execution, config import and arbitrary local-path Agent transfers remain administrator capabilities; browser file transfer stages only the uploaded or downloaded bytes. The gateway continues to own network-target policy, account permission changes and closing revoked WebSockets.
...
Enabling ordinary users to name Host key paths or agents would grant access to credentials belonging to the service account. Inline keys support key authentication without that access. Ordinary accounts must reference stored jump aliases on both mutation and connection; a deleted alias cannot become a DNS address that receives the target host’s credentials. Administrators retain address-based jumps and local proxy commands.
