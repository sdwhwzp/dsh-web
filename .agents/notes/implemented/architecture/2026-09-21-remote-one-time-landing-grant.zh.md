...
- **本机页面判定同样读取该钩子，但以页面来源为闸门（issue #1682）。** 客户端的
  「本页是否就是这台机器自己」判定（`src/remote-channel-rules.ts` 的 `isLocalPage`，
  并由内联 boot 脚本镜像）接受两类页面：协议不属于 web 方案（`WEB_PAGE_PROTOCOLS`：
  `http:`、`https:`，以及网页能铸造的文档——`blob:`、`data:`、`about:`、
  `filesystem:`）的页面，或传输已带 `ownsHost` **且**主机名是方案本地名（不含点、
  不含冒号）的页面。点名 web 一侧而不是点名各个外壳，让每一张本机应用交付的页面都留在栅栏
  之外：桌面外壳的 `dsh-app://app/`（方案白名单是最初的修法）、`file:` 页面，以及后续
  外壳引入的任何方案。协议缺失或为空不构成本机页面的证据——读不到时栅栏保持升起。
  这个「非网络来源」闸门是关键：落地页给已配对的局域网/隧道页面发布的是同一个钩子，而那种
  页面必须继续走门控通道——host 模式买到的是呈现面，绝不是配对门的豁免。原判定只看主机名，
  与官方客户端自相矛盾（官方本来就把 `ownsHost` 的应用壳视为本机），于是把整个桌面端挡在
  一页它永远配不成功的配对栅栏后面（外壳会丢掉每一个 `set-cookie`）。
...
- `tests/remote-channel.spec.ts` 与 `tests/remote-channel-boot.spec.ts` 用同一张表
  覆盖方案矩阵：回环、`dsh-app:`、未知应用方案、`file:`、读不到的方案、带钩子的方案本地
  名、局域网来源、隧道来源，以及网页铸造的 `blob:` 文档。boot 规格断言内联脚本与
  `isLocalPage` 在每一行上结论一致，两半不会再在交付方案上分叉。
...
- 把已知桌面外壳的方案列成"本机"（#1682 最初的修法）：否决——它只是修好了报障的那一个
  外壳，下一个（改名后的 `dsh-app:`、别家外壳的方案、`file:` 页面）依旧会被挡在一页它永远
  配不成功的配对栅栏后面。点名 web 一侧让未知的应用方案天然属于本机，而远端无法交付的方案
  本来就不是栅栏需要守的边界。
