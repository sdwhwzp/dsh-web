...
Amended 2026-09-10 (facts only): the aggregate pin moved to `dsh-better-sidebar@0.19.0-alpha.1`, the alpha npm published 2026-09-09 whose peers declare `^0.1.5-alpha.2` across the board — exactly the cohort this repo had already moved to, so no prerelease tolerance is involved. Compared with 0.18.0 its static imports add `@deepseek-ai/dsh-session`, and `dsh.client.inject` adds the optional `@deepseek-ai/dsh-client-ui-sidebar-right` face (published at 0.1.5-alpha.2, `peerDependenciesMeta` optional); its `cordis.patch.yml` is byte-identical to 0.18.0's, so the aggregate patch needed no regeneration. The `minimumReleaseAgeExclude` pin and the `scripts/e2e-mount.sh` scratch-profile heredoc entry moved to the exact new version (the heredoc had been left at 0.18.0-alpha.0 by the 0.18.0 bump). The desktop seed (`0.3.14` payload) still rides better-sidebar 0.18.0 and stays untouched; its ledger entry moves when a family publish re-rolls the seed.
...
Follow-up (2026-09-10, 0.19.0-alpha.1 round): `pnpm aggregate:check`, `pnpm typecheck`, `pnpm test`, `pnpm docs:check`, `pnpm i18n:check`, and `pnpm test:scripts` (269 tests) pass after the re-pin; `bash scripts/e2e-mount.sh` mounts the repacked aggregate into a scratch profile and the Playwright lane passes again (`better-sidebar` mounted, no crash markers, `archive-manager` absent).
