...
2026-09-10 修正（仅事实）：聚合 pin 移到 `dsh-better-sidebar@0.19.0-alpha.1`——npm 于 2026-09-09 发布的 alpha，peer 全面声明 `^0.1.5-alpha.2`，与本仓库已迁移到的 cohort 完全一致，不依赖任何预发布容差。相对 0.18.0，其静态 import 新增 `@deepseek-ai/dsh-session`，`dsh.client.inject` 新增 optional 的 `@deepseek-ai/dsh-client-ui-sidebar-right` 面（0.1.5-alpha.2 已发布该面，`peerDependenciesMeta` 标记 optional）；其 `cordis.patch.yml` 与 0.18.0 字节一致，聚合 patch 无需重新生成。`minimumReleaseAgeExclude` 引脚与 `scripts/e2e-mount.sh` scratch profile heredoc 条目随新精确版本更新（heredoc 在 0.18.0 那轮被遗留在了 0.18.0-alpha.0）。desktop seed（0.3.14 载荷）仍走 better-sidebar 0.18.0，保持不动；其台账条目待下次家族发包重排 seed 时再动。
