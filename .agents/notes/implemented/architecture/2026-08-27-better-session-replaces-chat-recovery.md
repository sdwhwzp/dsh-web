...
- Ingestion follows the established external-integration pattern used for `dsh-better-sidebar`: consume the upstream **npm package** (`@morlay/better-session@0.0.11`, registry-readable, SDK cohort rc.2-compatible), pinned in `packages/dsh-web-all/package.json`; no source is vendored into this repository.
