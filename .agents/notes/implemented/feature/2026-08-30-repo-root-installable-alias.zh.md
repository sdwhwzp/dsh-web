...
- 根 `package.json` 声明 `"dsh": { "bundle": { "patch": "./packages/dsh-web-all/cordis.patch.yml" } }`、随发版走的 `version`、`dependencies: { "@linxin666/dsh-web-all": "0.4.2" }`（精确对应同提交补丁的已发布聚合包，见[精确聚合包钉版](../../bug-fix/2026-09-10-root-alias-exact-aggregate-pin.md)），并用两项 `files` 白名单保证打包式 git 安装只携带清单；
...
- 每次家族发版都要把根的 `version` 与依赖一起钉到该版本，`scripts/verify-version.mjs` 在任一与 tag 不一致时直接让发布失败（见[精确聚合包钉版](../../bug-fix/2026-09-10-root-alias-exact-aggregate-pin.md)）；根包仍是 private，这个版本号只用于标注 git / link 安装读到的仓库清单。
