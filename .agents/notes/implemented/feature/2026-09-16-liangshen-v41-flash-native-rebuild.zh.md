...
- 插件的 Web 设置界面新增三个塑造 preset 的字段（`presentation`、`planningEffort`、`executionEffort`）。设置界面编辑的是插件自己的命名空间，而塑造会话的取值住在 preset 的行配置里；两者在**同步 preset 时**汇合——插件拷贝 bundle 的同时把这些取值写入产出的 `agent.cordis.yml`，因此真正被会话运行的是设置界面的取值。覆写按 `- id:` 标记定位行、只改已存在的键，组合里没有的键不会被凭空写入；比较也相应地以"应用覆写之后的源"为准，因此设置未变时报 `current` 而不是每次启动都重写。同步的 mtime 快路径保留给普通拷贝文件，只有组合文件按内容比较。
...
- 宿主重启时 preset 同步会覆盖 `~/.dsh/.agent-presets/liangshen`，升级后新建的会话组合出新的 wire 行为；早期修订持久化的历史保留其注入目录消息（`plugin` 来源 kind 不变），退役配置键只告警不报错。
