...
The account-scoped settings projection, override markers, reset defaults, and HTTP routes carry the same `bubbleScale` field. Updating another account setting preserves the stored multiplier, and account reloads retain it without sharing another user's display preferences.
