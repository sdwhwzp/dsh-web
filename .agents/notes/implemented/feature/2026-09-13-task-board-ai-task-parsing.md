...
The new-task form carries a paste box, a model picker, and one action button. The picker is built from the same `executionOptions.models` roster the task's own model pin uses, with the last browser selection remembered. An empty choice uses the Host default, and a remembered model absent from a populated roster falls back to that default. `POST {TASK_BOARD_API_PREFIX}/parse` sits behind the board's existing loopback and same-origin fence, accepts `{ text, model }`, and answers `{ ok: true, draft }` or a typed failure whose code maps onto a status: `no-model` 503, `parse-failed` and `model-error` 502, `timeout` 504. The client phrases each code in the active language, so no status code reaches the form.

The Host resolves the `llm` service lazily per request and deliberately does not list it in the plugin's `inject` array: a deployment without a model must still mount the whole board, and the route then answers `no-model` instead of the plugin failing to load. One `llm.stream` call runs on the qualified `provider/model` route with a system prompt that demands a single JSON object, a 45 s budget, and an abort wired to the client disconnecting. Extraction strips code fences and reads the first JSON object; an unusable reply falls back to the pasted text for the title and prompt, so nothing the user wrote is lost. Terminal model errors and aborts reject the parse even after partial text. A request already cancelled before dispatch never calls the model, and a stream that closes on cancellation cannot return a fallback draft.

The draft fills title, description, and run prompt only. Nothing reaches the ledger until the user submits the form, so a model cannot create or execute a task on its own. The explicit Create and run action saves through the Host before requesting execution; if the start is refused, it opens the saved task for the existing confirmation and error flow.

Deployments with account authentication await that authorization before parsing the request body or calling a model. A rejected or revoked account cannot consume model quota through the parse route, including while its asynchronous verifier is pending.
