...
- root `package.json` declares `"dsh": { "bundle": { "patch": "./packages/dsh-web-all/cordis.patch.yml" } }`, a `version` that follows the release version, `dependencies: { "@linxin666/dsh-web-all": "0.4.2" }` (the exact released aggregate matching the shipped patch — see the [exact aggregate pin](../../bug-fix/2026-09-10-root-alias-exact-aggregate-pin.md)), and a two-entry `files` whitelist so a packed git install ships the manifest and nothing else;
...
- The root version and its dependency are brought to the release version on every family release, and `scripts/verify-version.mjs` fails the publish when either drifts from the tag ([exact aggregate pin](../../bug-fix/2026-09-10-root-alias-exact-aggregate-pin.md)). The root stays private: its version only labels the repository manifest that a git or link install reports.
