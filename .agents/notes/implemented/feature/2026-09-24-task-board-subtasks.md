...
- The subtree is flattened into that one Team because only a Lead may spawn; a teammate name is the member title slug plus the run-group token, which keeps it unique inside the Team across repeated runs. A team run always mints a fresh Lead session (teammate names are immutable within one Team, so reusing an older session would collide), and the provider comes from the `teamProvider` config (default `spawn`).
