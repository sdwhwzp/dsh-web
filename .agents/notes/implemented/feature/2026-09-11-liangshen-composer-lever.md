...
- Pulling the lever down composes LiangShen mode for the current session; pushing it up selects the first available non-LiangShen preset from the remembered selection, deployment default, then roster order. An enabled lever with no restore target renders nothing, so a deployment default of LiangShen cannot leave a visible switch that only selects itself.
