...
- On the npm-published web runtime, gateway writes go through the official CLI. The gateway resolves `dsh` from the host process PATH first, then from `node_modules/.bin` project roots above the running host entry, and finally reuses a recognized official DSH Node entry with its active loader flags. This covers global, local-wrapper, npx, and source-checkout launches; writes remain unavailable when none exists. Installs of git sources can take minutes and run as background jobs. Gateway updates apply only to npm registry sources, resolve the latest version on the host, and succeed only after the same installed package reports that exact version.
