...
| 账号独立宠物 | 直连账号与每个已认证网关账号分别保存宠物选择、皮肤、名字、位置、可见性、开关、亲密度、小鱼干、玩法状态与互动计数；新网关账号默认关闭，可独立开启自己的宠物而不影响其他账号 |
...
- **挂载点**：已开启的账号在 `document.body` 挂载一个全局 React 根，因此无会话、新会话与会话中都能使用宠物；已关闭或账号设置仍在加载时不挂载宠物根或轮询。组件内部用 `createPortal` 渲染全局浮层。根容器随插件 fiber 生命周期走：fiber 销毁时卸载 React 根、移除容器并停止轮询与设置订阅；热重载或重复注入的新 bundle 接管页面级单挂载槽，页面始终只有一个 `[data-dsh-pet-root]`（issue #785）。
