...
| 账号独立宠物 | 直连账号与每个已认证网关账号分别保存宠物选择、名字、位置、可见性、开关、亲密度、小鱼干、玩法状态与互动计数；一个账号不能修改另一个账号的宠物，新账号默认关闭宠物 |
...
- **账号持久化**：直连宿主端口沿用 `$DSH_HOME/pet.json`。通过验证的网关身份 `(来源, 不变账号 ID)` 映射到 `$DSH_HOME/pet-accounts/` 下的 SHA-256 不透明目录；其中的 `pet.json` 保存该账号的宠物选择、名字、显示配置、开关、亲密度、小鱼干与互动计数。账号改名不会重置宠物。
