...
- **账号持久化**：直连宿主端口沿用 `$DSH_HOME/pet.json`。通过验证的网关身份 `(来源, 不变账号 ID)` 映射到 `$DSH_HOME/pet-accounts/` 下的 SHA-256 不透明目录；其中的 `pet.json` 保存该账号的宠物选择、按宠物保存的皮肤、名字、显示配置、开关、亲密度、小鱼干与互动计数。没有账号文件时从 `enabled: false` 开始；在「设置 > 宠物」开启后只为该身份持久化。账号改名不会重置宠物。
