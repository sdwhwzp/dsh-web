...
- **Account persistence**: direct access at the Host port keeps the established `$DSH_HOME/pet.json`. A verified gateway principal `(source, immutable id)` maps to an opaque SHA-256 directory under `$DSH_HOME/pet-accounts/`; its `pet.json` stores that account's selection, per-pet skin, names, display, switches, affinity, treats, and interaction counters. A principal without a file starts with `enabled: false`; enabling it in Settings > Pet persists only for that principal. Renaming an account does not reset its pet.
