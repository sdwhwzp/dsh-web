...
| Per-account companions | Direct access and every authenticated gateway account keep independent selection, skin, name, position, visibility, switches, affinity, treats, gameplay state, and interaction counters; a new gateway account starts disabled and can enable its own pet without changing another account |
...
- **Mount point**: an enabled account mounts one global React root on `document.body`, so the pet remains available with no session, on a new session, and mid-session; a disabled or still-loading account mounts no pet root or poll loop. The component uses `createPortal` internally to render the global floating layer. The root follows the plugin fiber lifecycle: fiber disposal unmounts the React root, removes the container, and stops the poll loop and settings subscription; a hot-reloaded or re-injected bundle takes over the page-global slot, so the page always holds exactly one `[data-dsh-pet-root]` (issue #785).
