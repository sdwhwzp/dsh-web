...
> 注（DSH 0.1.2-alpha.2）：alpha.2 官方移除了 `dsh-better-sidebar` 依赖的 `@deepseek-ai/dsh-client-runtime` 面，故 2026-08-30 曾暂时排除；现已回归聚合，当前钉在 0.19.0-alpha.1（2026-09-09 发布、对齐 0.1.5-alpha.2 cohort 的 alpha 构建，inject 名单含 `@deepseek-ai/dsh-client-modules` 与 optional 的 `@deepseek-ai/dsh-client-ui-sidebar-right` 面）。`@mlgbnb/dsh-archive-manager` 仍排除：其最新上游构建（1.0.7）仍 import 已移除面，保留会导致 `dsh web` 启动失败。

## 是什么

- **一次安装、全部到位**：其 dependencies 引入家族的全部子插件包（任务看板 / Git 图谱 / 宠物 / 移动端远程 / SSH / 模型能力 / 皮肤 / 设置区 / 社区插件 等，完整清单以 `aggregate.yml` 为准），外加外部 npm 插件 `dsh-better-sidebar`（默认右侧面板：文件资源管理器 / 编辑器 / 终端 / Git / 浏览器；当前挡位钉在 0.19.0-alpha.1，对齐 0.1.5-alpha.2 cohort）。`@mlgbnb/dsh-archive-manager`（社区归档管理：按项目分组、搜索筛选、预览对话、一键恢复与删除）未内置——其上游构建仍 import 已移除的 `@deepseek-ai/dsh-client-runtime` 面。
...
- **右侧面板**：右侧面板固定为 `dsh-better-sidebar`，聚合包的浏览器兼容层为带非空文件路径的编辑器提供直接下载入口，调用认证的 `/sidebar/file` 路由并在点击时解析当前会话；better-sidebar 自带同类入口时不会重复添加。其偏好在 [dsh-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar) 自身的设置区管理。
...
- `dsh-better-sidebar@0.19.0-alpha.1` 是外部 npm 依赖（非本仓库出品），本包发版前必须先发布它（发布顺序见 `docs/publish-prep.md`）。
