...
- **Right panel**: the right panel is always `dsh-better-sidebar`; the aggregate browser compatibility layer adds a direct download for editors with a nonempty file path, using the authenticated `/sidebar/file` route and resolving the current session on click; it skips editors that already provide a download. Its preferences live in [dsh-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar)'s own settings section.
