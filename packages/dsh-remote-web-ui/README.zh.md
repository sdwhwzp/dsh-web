...
> 为 dsh web GUI 提供共享**同一份界面**的远程访问：通过仅限回环的控制 API 签发一次性链接来配对手机或另一台电脑，两端运行的都和本机一样是官方 Web GUI——手机获得注入的竖屏触控适配，电脑获得完整桌面；准入基于一次性配对令牌与可撤销设备会话。设置卡片可将服务绑定到局域网，可选 Cloudflare 快速隧道触达公网。桌面侧边栏有意不显示远程访问入口与更新入口。
...
- **桌面侧边栏**：不注册 `sidebar.remote` 或 `sidebar.footer.action` 贡献，因此左下角既没有远程访问入口，也没有更新入口。配对路由、现有已配对设备会话、远程通道和更新路由保持运行。
- **配对控制 API**：仅限回环的 `/api/pair/issue` 端点为手机和电脑创建同一类一次性链接；同族的状态、事件、撤销与停止端点展示并控制实时设备状态。未配对调用方看不到作为会话凭据的设备 id 或原始 User-Agent。
- **手机侧**：打开已签发链接后以一次性限时令牌绑定并重载进入**官方 Web GUI**——不存在会漂移的第二套界面。手机竖屏时插件在运行中的界面上注入触控适配层（见下文）。链接同时携带 harness 浏览器认证令牌，从未见过本服务器的设备一次导航即可完成认证与配对。
- **电脑侧**：同一链接在另一台电脑的浏览器里打开完整桌面 Web GUI，流量走门控 `/remote` 通道；未配对的电脑看到引导式拦截页（含手动粘贴配对令牌输入框），其后没有任何工作区数据。
- **安全**：同时只有一枚一次性令牌（签发新链接使旧链接失效；令牌被接受后不可复用；令牌会过期）。停止端点会撤销所有配对设备与当前令牌——`/remote` 通道在下一次请求即切断它们。配对是本插件对 `/remote` 通道的访问控制；暴露到局域网后的直连 `/api` 由 harness 围栏 + 浏览器认证约束（见安全模型）。回环（127.0.0.1）继续直接使用 `/api`。配对设备是**完全控制凭据**（见安全模型）。
...
- **实时状态**：仅限回环的状态与事件端点报告已连接设备；`/api` 姿态探测报告 SDK 栅栏仍敞开的 `/api` 来源。不注册桌面徽标。
- **自更新 API**：仅限回环的更新端点检查 dsh-web 新版本并执行带校验的更新。不注册桌面更新触发器或面板。
...
- 局域网使用：在设置卡片打开**局域网访问**（写入绑定块，自下一次 `dsh web` 启动生效），或以 `dsh web --host 0.0.0.0` 启动。默认 `127.0.0.1` 绑定时，签发端点返回明确的 `lan-required` 响应，除非已配置公网地址（见下文）。签发与停止端点设计上仅限回环；来自局域网来源浏览器的请求会被拒绝。
...
重启 profile（`dsh web`）。插件的 `cordis.patch.yml` 插入单一插件行，同时挂载两个半区但不添加桌面侧边栏底部动作；通过下文的仅限回环控制 API 签发配对链接。
...
3. 在手机打开该链接：设备一次导航中越过 harness 浏览器认证门与配对门，重载后进入**官方 Web GUI**。手机上竖屏适配层已生效——与桌面同布局、同实时状态。
4. **改为配对电脑**：复制同一链接，在另一台电脑的浏览器打开。相同往返后完整 Web GUI 在彼处经门控 `/remote` 通道运行；未配对电脑看到引导式拦截页，其后没有数据。一枚令牌配一台设备；下一台设备请签发新链接。
5. 从回环请求 `GET /api/pair/status` 查看配对状态；使用仅限回环的撤销与停止端点撤销单台或全部设备。
...
在设置卡片打开**自动公网隧道**（autoTunnel）。插件运行自己的 Cloudflare 快速隧道（`cloudflared` 随包分发），把铸造的公网 URL 动态喂给配对链接与配对栅栏，并同步姿态探测——手机随时随地都能配对。开启期间忽略下方手动公网地址。
...
然后把打印的 URL 填入公网地址。配对链接由它构造，插件的配对栅栏接受该隧道 authority。姿态探测持续审计 SDK 栅栏对哪些 `/api` 来源保持敞开。
...
公网路径是同一条经隧道的往返（见「通过互联网远程访问」）：回环铸造 → 设备打开公网配对 URL → 接受 → 官方 UI。只有 `publicBaseUrl`（插件配置）命名隧道主机；`--trusted-host` 不属于本配对流。配对控制仍位于 `http://127.0.0.1` 的回环 API。
...
- **控制端点仅限回环**：签发/停止/撤销、设备事件流、lan-bind 状态与更新端点只应答回环。局域网来源浏览器会收到禁止响应。
- **撤销按请求生效**：停止落地时已在途的请求会完成；下一个请求 403。
- **配对设备会话默认持久化**：设备会话（非一次性配对令牌）写入 `$DSH_HOME/remote-web-ui-devices.json`（0600，临时文件 + 原子改名）。`dsh web` 重启后配对 cookie 依然有效。签发新链接会铸造新令牌；重启不会恢复当前令牌。空闲超过 `idleExpireMs`（默认 7 天）的会话被删除并须重新配对。设备 id 即会话凭据。需要时可用 `devicesFile` 指定其他绝对路径。更换 `cookieName` 会使现有设备失效（预期行为）。
...
`qrcode.react`（MIT，活跃维护，支持 React 16–19）仍是可复用配对面板组件的 SVG 渲染器，不过本 fork 不会在桌面侧边栏挂载该组件。`schemastery` 是 DSH 标准配置 schema 校验器。
