...
> Pairing-based remote access for phones and computers, with loopback-only update and pairing control APIs. The desktop sidebar intentionally shows neither a Check for updates action nor a Remote access action.
...
`/api/update` surface; the browser half accepts pairing links, runs the paired
remote channel and mobile presence flow, and renders the plugin settings card.
It does not register controls in either desktop sidebar footer slot.

## What it does

- **Desktop sidebar**: no `sidebar.remote` or `sidebar.footer.action` contribution is registered, so the lower-left area contains neither the update action nor the remote-access action. Pairing routes, existing paired-device sessions, the remote channel, and update routes remain active.
- **Pairing control API**: loopback-only `/api/pair` endpoints issue separate phone (`/m/?pair=...`) and computer (`/?pair=...`) links, report live device status, revoke individual devices, and stop all remote access. Credential-bearing device ids and raw User-Agent values are not exposed to unpaired callers. Both links share one single-use token, so pairing either device invalidates the other link.
...
- **Posture probe**: the plugin probes the SDK `/api` fence with forged
  ...
  origins are retried.
- **Live status**: the loopback-only pairing event stream reports the pairing
  state (waiting → connected → disconnected), including the authorized-device
  list.
- **Remote update**: the loopback-only `/api/update/status` and
  `/api/update/run` endpoints probe the npm registry for the installed
...
- For LAN use the server must be reachable from the phone: start with
  `dsh web --host 0.0.0.0`. With the default `127.0.0.1` bind, issuing a
  pairing link fails with an explicit LAN-reachability response unless a
  public base URL is configured (see "Remote access over the internet"
  below). Pairing-link issuance and revocation endpoints are loopback-only by
  design; call them through `http://127.0.0.1` and open the returned link on
  the remote device.
...
Restart the profile (`dsh web`). The plugin's `cordis.patch.yml` inserts the
single plugin row that mounts both halves; it does not add a desktop sidebar
footer action.
...
2. On the primary computer, issue a fresh one-time pairing link through the
   loopback-only control endpoint:
   `curl -X POST http://127.0.0.1:3080/api/pair/issue -H 'content-type: application/json' -d '{}'`.
3. Open the returned `url` on the phone: the phone binds and
 ...
 4. On a secure origin, install the `/m/` page from the browser. The installed app retains the same mobile remote capabilities; when its browser context has no paired-device cookie, paste a fresh administrator-issued pairing link into its pairing screen.
 5. **To pair a PC instead**: use the returned token with the same origin and
    open the desktop URL form (`/?pair=<token>`, not `/m/`) in the other
    computer's browser.
    ...
    data. One active token pairs one device; issue a fresh link for the
    next device.
 6. Issuing another link invalidates the previous token. Posting to the
    loopback-only `/api/pair/stop` endpoint revokes every paired device; their
    next request, including the live stream, receives 403.
...
profile patch (or the plugin settings card — it hot-reloads). Issue a pairing
link through the loopback endpoint and open it from anywhere: the phone binds,
reloads into the mobile surface, and heartbeats keep it online. A
...
- **Sidebar slot types** (packages/client/ui-sidebar): retained only for the
  exported control-component prop types; this package registers no footer
  contribution.
...
2. From the primary host, post `{}` to the loopback-only
   `http://127.0.0.1:3190/api/pair/issue` endpoint and copy the returned token.
...
4. Confirm the loopback status stream reports the device as connected; a
   LAN-origin desktop opens no privileged pairing status stream.
5. Post to the loopback-only `/api/pair/stop` endpoint and confirm the phone's
   next `/api` request 403s (reconnect loops retry until a fresh token re-pairs).
...
`--trusted-host` is not part of this pairing flow. Token issuance and remote
revocation remain loopback-only.
