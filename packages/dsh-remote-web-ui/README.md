...
- **Pairing control API**: the loopback-only `/api/pair/issue` endpoint creates the same single-use link for phones and computers; sibling status, event, revoke, and stop endpoints expose and control live device state. Credential-bearing device ids and raw User-Agent values are never exposed to unpaired callers.
- **Phone side**: opening the issued link binds the phone with a one-time, time-limited token and reloads into the **official Web GUI** — there is no second interface to drift from. While the phone is in portrait, the plugin injects a touch-adaptation layer over the running UI (see "The mobile adaptation layer" below). The link carries the harness browser-auth token as well, so a device that has never seen this server authenticates and pairs in one navigation.
...
3. Open the link on the phone: the device clears the harness browser-auth gate and the pairing gate in one navigation, reloads, and boots the **official Web GUI**. On a phone, the portrait adaptation layer is already active — same layout as the desktop, same live state.
4. **To pair a PC instead**: copy the same link and open it in a browser on the other computer. After the same round trip the full Web GUI runs there over the gated `/remote` channel; unpaired PCs see the guided blocking page and no data. One active token pairs one device; issue a fresh link for the next device.
...
- **Control endpoints stay loopback-only**: issue/stop/revoke, the device event stream, the lan-bind status, and the update endpoints answer only to loopback. A LAN-origin browser receives a forbidden response.
