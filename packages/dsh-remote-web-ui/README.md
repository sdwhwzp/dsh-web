...
- **Desktop sidebar**: no `sidebar.remote` or `sidebar.footer.action` contribution is registered, so the lower-left area contains neither the remote-access action nor the update action. Pairing routes, existing paired-device sessions, the remote channel, and update routes remain active.
- **Pairing control API**: the loopback-only `/api/pair/issue` endpoint creates the same time-limited link for phones and computers; sibling status, event, revoke, and stop endpoints expose and control live device state. Credential-bearing device ids and raw User-Agent values are never exposed to unpaired callers.
- **Phone side**: opening the issued link binds the phone and boots the **official Web GUI** — there is no second interface to drift from. The accept chain is cookieless (`/pair-accept` → `/pair-app`), so the phone needs no harness browser-auth cookie. On https origins a reopen service worker sends later history, bookmark, and tab-restore navigations back through the paired application shell. While the phone is in portrait, the plugin injects the touch-adaptation layer described below.
...
- **Live status**: loopback-only status and event endpoints report connected devices; an `/api` posture probe reports any host whose `/api` fence the SDK leaves open. No desktop badge is registered.
- **Self-update API**: loopback-only update endpoints check for a newer dsh-web release and run the verified update. No desktop update trigger or panel is registered.
...
- For LAN use, either flip the **局域网访问** toggle in the settings card (writes the bind block; effective from the next `dsh web` start) or start with `dsh web --host 0.0.0.0`. With the default `127.0.0.1` bind, the issue endpoint returns an explicit `lan-required` response unless a public base URL is configured (see "Remote access over the internet" below). The issue and stop endpoints are loopback-only by design; requests from a LAN-origin browser are refused.
...
Restart the profile (`dsh web`). The plugin's `cordis.patch.yml` inserts the single plugin row that mounts both halves without adding desktop sidebar footer actions; issue pairing links through the loopback control API described below.
...
5. Query `GET /api/pair/status` from loopback to inspect paired state; use the loopback revoke and stop endpoints to revoke one device or every device.
...
While the quick tunnel runs, the plugin additionally registers its current tunnel URL with the dsh-market relay registry and builds the pairing link on a fixed origin — `https://<id>.dsh-market.com` — minted once per profile and stored under `$DSH_HOME/remote-web-ui-registry/`. The hostname never changes, so the device's bookmark and pairing cookie keep working across `dsh web` restarts with no Cloudflare account, no dashboard, and no domain of your own.

- Registration is secret-authenticated: the plugin mints a 256-bit secret next to the identity and re-syncs the mapping on every tunnel start (crash restarts included) with capped backoff. If the registry is unreachable, the pairing link falls back to the raw quick URL for that session and the status endpoint reports it.
- A device opening the stable origin while the instance is offline sees an explicit "instance offline — refresh in a moment, no re-pairing needed" page instead of a dead link.
...
The relay above already fixes the device's origin on a shared dsh-market subdomain. A Cloudflare named tunnel is the alternative for deployments that want a hostname on **their own** domain: its public hostname is permanent, so the device's bookmark and pairing cookie keep working across `dsh web` restarts — pair once, never again (until the session idles past `idleExpireMs`).
...
Then set the printed URL as 公网地址. The pairing link is built from it, and the plugin's pairing fence accepts the tunneled authority. The posture probe keeps auditing which `/api` authorities the SDK fence leaves open.
...
Pinned to the 0.1.2-alpha.3 line; the seams this build relies on:

- **No desktop footer contribution**: this fork intentionally injects neither `sidebar.remote` nor `sidebar.footer.action`; pairing and update stay available through loopback-only APIs.
...
The fence helpers (`isTrustedApiRequest` / `isLoopbackHostname`) are reimplemented locally in `src/gate.ts` / `src/routes.ts`: the connection plugin stopped exporting them, so the pairing routes carry their own copy scoped to the literals the pairing links advertise.
...
2. From loopback, `POST http://127.0.0.1:3191/api/pair/issue` with `{}` and capture the returned `<lan-url>/pair-accept?pair=<token>` link; confirm the desktop sidebar has no remote-access or update action.
3. In a second tab under mobile emulation (390x844, touch) open that link: the chain `/pair-accept → /pair-app?device=<id> → /` sets the device cookie, serves the patched official shell, and boots the UI — `document.body.classList` carries `dsh-remote-portrait`, the adaptation stylesheet and the whale button exist, and `__DSH_TRANSPORT__.ownsHost` is `true`. The settings surface renders host data (host mode), not a memory mirror. In an https deployment the shell also registers `/pair-app.sw.js`; reloading `/` in that tab boots the app instead of the harness 401.
4. `GET /api/pair/status` from loopback reports the connected device; the desktop sidebar remains unchanged, and a LAN-origin page cannot open the loopback-only status stream.
5. `POST /api/pair/stop` from loopback cuts the device off: its next request 403s with `unpaired` (the fence page offers the manual pair-token input).

The public path is the same round trip through a tunnel (see "Remote access over the internet"): loopback mint → device opens the public pairing URL → accept → official UI. Only `publicBaseUrl` (plugin config) names the tunneled host; `--trusted-host` is not part of this pairing flow. Pairing control remains on the loopback API at `http://127.0.0.1`.
...
- **Control endpoints stay loopback-only**: issue/stop/revoke, the device roster and event stream, the lan-bind status, and the update endpoints answer only to loopback. A LAN-origin browser receives a forbidden response.
- **The app landing is cookieless-optional.** After pairing, the issued link lands the device on `/pair-app`, served by this plugin: the official shell is delivered without passing the harness index gate, and the device credential rides a `x-dsh-remote-device` header (fetch) / `device` query (WebSocket upgrades) that the boot patch attaches from sessionStorage. The flow therefore works when the device browser blocks all cookies; the pairing cookie remains the primary credential where the browser stores it, and the harness browser-auth cookie is no longer required on the phone path at all.
...
- **Paired device sessions persist by default**: device sessions (not the pairing token) are written to `$DSH_HOME/remote-web-ui-devices.json` (0600, temp file + atomic rename). A paired cookie still works after a `dsh web` restart. Issuing a new link still mints a new token; restarting does not restore the current token. Sessions idle for `idleExpireMs` (default 30 days; the reopen service worker refreshes the window on every navigation it serves) are deleted and must pair again. Device ids are session credentials. Override `devicesFile` with another absolute path when needed. Changing `cookieName` invalidates existing devices (expected).
...
`qrcode.react` (MIT, actively maintained, React 16–19 support) remains the SVG renderer for the reusable pairing-panel component, although this fork does not mount that component in the desktop sidebar. `schemastery` is the DSH-standard config schema validator.
