...
- **Desktop sidebar**: no `sidebar.remote` or `sidebar.footer.action` contribution is registered, so the lower-left area contains neither the remote-access action nor the update action. Pairing routes, existing paired-device sessions, the remote channel, remain active.
...
- **Self-update**: the separate `dsh-update` plugin owns update endpoints and the trigger. This fork disables its aggregate row by default; private deployment artifacts are upgraded through the deployment process.
- **Desktop foot row**: the wide sidebar seats Settings and footer actions on one line, with full-width blocks such as the usage glance above it; the 56px rail retains the shell’s stacked footer.
...
- **No desktop footer contribution**: this fork intentionally injects neither `sidebar.remote` nor `sidebar.footer.action`; pairing stays available through loopback-only APIs.
