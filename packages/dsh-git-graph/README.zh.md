...
- **自动隔离**（`autoIsolate`）：git 工作区的“新会话”动作会静默创建全新托管 worktree 并在其中开会话——Claude 桌面版式的自动形态。`autoBaseline` 选择基线：当前检出 HEAD（`current`）或远程默认分支（`default`，解析为 `origin/HEAD`，无远程时回退 HEAD）。已在托管目录下的工作区不会被二次隔离；任何失败都降级回官方新会话行为。浏览器半区包装 `uiWorkspace.startSession` 方法，工作区服务负责注册和目录操作。直接连接工作区仍保持正常的复用行为；导航或工作区服务不兼容时，自动隔离会关闭并打印诊断。
