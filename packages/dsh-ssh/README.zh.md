...
账号模式下，普通用户通过密码或粘贴自己的私钥认证。Host 私钥路径、Host SSH agent、本机 `ProxyCommand` 执行和导入服务器 `~/.ssh/config` 仅供管理员使用。使用这些凭据或 `ProxyCommand` 的旧账号连接仍会列出，但连接前须编辑。GUI 传输使用浏览器文件选择器；普通账号不能通过 `ssh_upload` 或 `ssh_download` 操作任意 Host 文件路径。
