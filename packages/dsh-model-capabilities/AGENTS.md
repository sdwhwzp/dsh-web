...
- host 半区只注册本包命名空间 `dsh-model-capabilities`(禁用存档),经
  `src/mount-once.ts`(sync-shared 生成副本)防双源重复注册——settings 命名空间
  重复注册会 fail loud;能力读写全部走 `remote.settings.describe/mutate` 官方线路,
  目标是 `llm-pi-ai` 命名空间。
...
- **刷新只跟两个命名空间**:`settings/document-updated` 仅当 `llm-pi-ai` 或本包
  存档命名空间变化时刷新;并发 `describe` 合并为一次 wire 调用;后台刷新保留
