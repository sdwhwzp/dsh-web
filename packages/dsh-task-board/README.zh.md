...
- **粘贴文本 AI 解析（issue #1540）**：新建任务表单可以把从别处复制的一段话交给模型，整理成标题、描述和执行 Prompt。模型取当前部署已配置的模型（与任务「模型」下拉同一份列表；浏览器记住解析模型，没有可用偏好时使用宿主默认模型），调用在宿主侧完成，走看板既有的回环与同源校验，预算 45 秒并支持取消。解析结果只填进表单：不提交就不会创建任务，解析失败也不会改动你已经输入的内容。 Provider 的错误或中止结束事件按失败处理，即使此前已经返回部分文本；原文回退仅用于成功回复。
...
- **创建并运行**：新建任务表单可通过一次明确操作保存并启动任务。启动被拒绝时，打开已保存的任务以完成原有权限确认或查看错误；普通「创建」只保存、不执行。
