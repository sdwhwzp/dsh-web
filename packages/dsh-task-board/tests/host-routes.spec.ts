...
import { makeTaskBoardRoutes, type TaskBoardRouteAccess } from '../src/host-routes.ts'
...
describe('task-board parse route (#1540)', () => {
  ...
  async function start(options: { parseTask?: unknown; omit?: boolean; authenticate?: TaskBoardRouteAccess['authenticate'] } = {}): Promise<void> {
    const service = {
      snapshot: () => snapshot,
      apply: () => snapshot,
      subscribe: () => () => undefined,
    ...
    const routes = makeTaskBoardRoutes(service, { authenticate: options.authenticate }, routeOptions)
    server = createServer((req, res) => {
      const route = routes.find(candidate => candidate.path === new URL(req.url ?? '/', 'http://local').pathname)
      if (route === undefined) { res.writeHead(404); res.end(); return }
      void route.handler(req, res)
